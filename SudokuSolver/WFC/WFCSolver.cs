...
{
  public class WFCSolver : AbstractBoard<WFCState>
  {
    ...
    public WFCSolver(IEnumerable<WFCState> values, IValidator<WFCState> validator, int width, int height, int rectsPerSide) : base(values, new SudokuElementFactory<WFCState>(rectsPerSide, validator), width, height)
    {
      ALL_VALUES = Enumerable.Range(1, width*height).ToArray();
      RectanglesPerSide = rectsPerSide;
    }

    public WFCSolver(Func<int, int, WFCState> initializer, IValidator<WFCState> validator, int width, int height, int rectsPerSide) : base(initializer, new SudokuElementFactory<WFCState>(rectsPerSide, validator), width, height)
    {
      ALL_VALUES = Enumerable.Range(1, width*height).ToArray();
      RectanglesPerSide = rectsPerSide;
    }
